...
function getOptions(q, refreshUrl, token) {
    var query = refreshUrl || '?q=' + encodeURIComponent(q) + '&result_type=recent&include_entities=false&count=100';
...
function search(q) {
    redisClient.get('refreshUrl', function(err, result) {
        var refreshUrl = result;
        ...
        setInterval(function() {
            getTweets(q, refreshUrl, function(data) {
                ...
                console.log('found ', result.statuses.length, ' results');
